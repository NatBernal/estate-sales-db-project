CREATE OR REPLACE PACKAGE PKG_VENTAS_INMUEBLES AS
    ...
    PROCEDURE SP_POBLAR_PUEBLOS (
        P_FECHA_CARGA IN DATE DEFAULT SYSDATE
    );
    
    PROCEDURE SP_POBLAR_TIPOS_PROPIEDAD (
        P_FECHA_CARGA IN DATE DEFAULT SYSDATE
    );
    
    PROCEDURE SP_POBLAR_TIPOS_RESIDENCIA (
        P_FECHA_CARGA IN DATE DEFAULT SYSDATE
    );
    
    PROCEDURE SP_POBLAR_LOCALIZACIONES (
        P_FECHA_CARGA IN DATE DEFAULT SYSDATE
    );
    
    PROCEDURE SP_POBLAR_PROPIEDADES (
        P_FECHA_CARGA IN DATE DEFAULT SYSDATE
    );
    
    PROCEDURE SP_POBLAR_VENTAS (
        P_FECHA_CARGA IN DATE DEFAULT SYSDATE
    );
    
    PROCEDURE SP_POBLAR_OBSERVACIONES (
        P_FECHA_CARGA IN DATE DEFAULT SYSDATE
    );

    PROCEDURE SP_LIMPIAR_BASE_DATOS (
        P_FECHA_LIMPIEZA  IN DATE DEFAULT SYSDATE,
        P_LIMPIAR_CONTROL IN BOOLEAN DEFAULT FALSE
    );
    
    PROCEDURE SP_POBLAR_TODO (
        P_FECHA_CARGA IN DATE DEFAULT SYSDATE
    );
    ...
    FUNCTION FN_VALIDAR_FECHA_CARGA (
        P_FECHA_CARGA IN DATE
    ) RETURN BOOLEAN;

END PKG_VENTAS_INMUEBLES;
/

CREATE OR REPLACE PACKAGE BODY PKG_VENTAS_INMUEBLES AS
        ...
        INSERT INTO PUEBLOS ( NOMBRE )
            ...
            WHERE
                ...
                AND NOT EXISTS (
                    SELECT 1
                    FROM PUEBLOS P
                    WHERE UPPER(P.NOMBRE) = UPPER(T.PUEBLO)
                );
        ...
        INSERT INTO TIPOS_PROPIEDAD ( DESCRIPCION )
            ...
            WHERE
                ...
                AND NOT EXISTS (
                    ...
                    WHERE UPPER(TP.DESCRIPCION) = UPPER(TABLA_PRUEBA.TIPO_PROPIEDAD)
                );
    ...
    BEGIN
        INSERT INTO TIPOS_RESIDENCIA ( DESCRIPCION )
            ...
            WHERE
                ...
                AND NOT EXISTS (
                    SELECT 1
                    FROM TIPOS_RESIDENCIA TR
                    WHERE UPPER(TR.DESCRIPCION) = UPPER(TABLA_PRUEBA.TIPO_RESIDENCIA)
                );
    ...
    BEGIN
        INSERT INTO LOCALIZACIONES (
            ...
            SELECT
                P.ID_PUEBLO,
                EXTRACT_COORDINATES(T.COORDENADAS, 1),
                EXTRACT_COORDINATES(T.COORDENADAS, 0),
    ...
    BEGIN
        INSERT INTO VENTAS (
            ...
            WITH PROPIEDADES_POR_PUEBLO AS (
                ...
                SELECT
                    ...
                    OVER(PARTITION BY NUMERO_SERIAL, PUEBLO
                         ORDER BY FECHA_REGISTRO DESC, ANIO_VENTA DESC
                    ) AS RN_PRUEBA
                FROM
                    TABLA_PRUEBA
                WHERE
            ...
            SELECT
                ...
                PU.VALOR_VENTA,
                CASE
                    WHEN REGEXP_LIKE ( TRIM(PU.RELACION_VENTA), '^-?[0-9]+\.?[0-9]*$' ) THEN
                        TO_NUMBER(REPLACE(TRIM(PU.RELACION_VENTA), '.', ','))
                    ELSE
                        ROUND(TO_NUMBER(REPLACE(PU.RELACION_VENTA, '.', '')) / 1000000000, 3)
                END,
                PU.COD_NO_USO
            FROM
                     PRUEBA_UNICA PU
    ...
    PROCEDURE SP_LIMPIAR_BASE_DATOS (
        ...
        TYPE T_TABLAS IS TABLE OF VARCHAR2(50);
        V_TABLAS           T_TABLAS := T_TABLAS('OBSERVACIONES', 'VENTAS', 'PROPIEDADES', 
                                      'LOCALIZACIONES', 'TIPOS_RESIDENCIA',
    ...
    BEGIN
        FOR I IN 1..V_TABLAS.COUNT LOOP
            ...
            IF V_FILAS_ELIMINADAS > 0 THEN
                ...
                DBMS_OUTPUT.PUT_LINE('Tabla ' || V_TABLA || ': ' || V_FILAS_ELIMINADAS || ' registros eliminados');
            ELSE
                DBMS_OUTPUT.PUT_LINE('Tabla ' || V_TABLA || ': ya estaba vacía');
            END IF;
        END LOOP;
    ...
    BEGIN
        ...
        V_LONGITUD := REGEXP_SUBSTR(P_LOCATION, 'POINT\s*\(([-0-9.]+)\s+([-0-9.]+)\)', 1, 1, 'i', 1);
        V_LATITUD := REGEXP_SUBSTR(P_LOCATION, 'POINT\s*\(([-0-9.]+)\s+([-0-9.]+)\)', 1, 1, 'i', 2);
    ...
    BEGIN
        IF P_FECHA_CARGA IS NULL THEN
            RAISE_APPLICATION_ERROR(C_FECHA_NULA, 'La fecha de carga no puede ser nula');
        END IF;

        v_dia_semana := TO_NUMBER(TO_CHAR(P_FECHA_CARGA, 'D'));
        IF v_dia_semana IN (1, 7) THEN
            RAISE_APPLICATION_ERROR(C_DIA_NO_HABIL, 'No se permite cargar datos en fines de semana');
        END IF;

        v_hora := TO_NUMBER(TO_CHAR(P_FECHA_CARGA, 'HH24'));
        IF v_hora < 8 OR v_hora > 18 THEN
            RAISE_APPLICATION_ERROR(C_HORARIO_LABORAL, 'Solo se permite cargar datos en horario laboral (8:00 - 18:00)');
        END IF;

        v_diferencia_dias := TRUNC(P_FECHA_CARGA) - TRUNC(SYSDATE);
        IF v_diferencia_dias < -30 THEN
            RAISE_APPLICATION_ERROR(C_FECHA_ANTERIOR, 'No se permite cargar datos con fecha superior a 30 días anterior');
        END IF;

        IF v_diferencia_dias > 7 THEN
            RAISE_APPLICATION_ERROR(C_FECHA_POSTERIOR, 'No se permite cargar datos con fecha superior a 7 días posterior');
        END IF;
